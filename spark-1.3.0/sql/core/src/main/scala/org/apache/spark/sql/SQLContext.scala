...
class SQLContext(@transient val sparkContext: SparkContext)
   ...
   * 设置spark sql的参数,底层将配置信息放入到SQLConf对象中
   * @group config
   */
  ...
  @transient
  /**
   * QueryExecution实际执行SQL语句的第一步。
   * 用SqlParser解析的纯逻辑词法和语法封装的语法树(Unresolved LogicalPlan), 调用
   * Analyzer的apply方法生成一个Resolved LogicalPlan
   */
  ...
  @transient
  /**
   * SqlParser本质上是SparkSQLParser的实例。
   * SparkSQLParser封装了catalyst的SqlParser
   */
  ...
  /**
   * parseSql方法,是SqlParser执行的入口。
   * 方法调用了SqlParser的apply方法,创建一个SQL语句解析的LogicalPlan
   */
   ...
   */
  /**
   * 使用spark执行一条SQL查询语句,返回一个DataFrame。
   * SQL解析使用的方言,通过spark.sql.dialect设定。
   */
  def sql(sqlText: String): DataFrame = {
    // 通过SQLContext.setConf方法设定的参数spark.sql.dialect,判断是否为sql方言
    if (conf.dialect == "sql") {
      /**
       * spark sql具有lazy特性。
       * 也就是, sql方法执行一条sql语句时,默认只会调用SqlParser组件生成一个Unresolved LogicalPlan。
       * 然后将Unresolved LogicalPlan和SQLContext实例封装为一个DataFrame。
       * 
       * 针对DataFrame,可以执行一系列的show、select、groupBy等操作,或者将其转换为rdd。
       * 
       * 针对DataFrame执行完操作之后,才会触发spark sql后续的sql执行流程：
       * Analyzer、Optimizer、SparkPlan、Execute PhysicalPlan
       * 
       * 底层调用SqlParser长度apply方法,由SqlParser对sql语句通过内部各种select、insert词法、
       * 语法解析器执行解析,然后将sql组装成一个LogicalPlan语法树。语法树不包含任何其他的数据来源,
       * 因此该树名为：Unresolved LogicalPlan
       */
  ...
  protected[sql] class QueryExecution(val logical: LogicalPlan) {
    ...
    // 只需要用Unresolved LogicalPlan构造一个QueryExecution实例对象,就会继续出发后面的操作

...
private[sql] abstract class AbstractSparkSQLParser
  ...
  /**
    *  调用SqlParser的apply方法,将SQL解析为LogicalPlan时,会调用到SqlPaserr父类
    *  AbstractSparkSQLParser的apply方法。
    */
  def apply(input: String): LogicalPlan = {
    // Initialize the Keywords.
    lexical.initialize(reservedWords)
    /**
     * 用lexical.Scanner针对SQL语句进行语法检查和分析。
     * 解析包括：词法解析(将SQL解析为短语)、语法解析。
     * 
     * 最后生成一个Unresolved LogicalPlan。
     * 
     * 该LogicalPlan仅仅针对SQL语句本身,生成纯语法、不涉及任何关联的数据源信息
     */

...
private[spark] class MemoryStore(blockManager: BlockManager, maxMemory: Long)
  ...
  /**
   * 维护一个LinkedHashMap, 存储着block数据。
   * 每个block在内存中存放的数据, 存放在MemoryEntry容器中。
   */
  ...
  override def getBytes(blockId: BlockId): Option[ByteBuffer] = {
    ...
    // 如果Block支持序列化,那么调用BlockManager的dataSerialize方法序列化之后返回
          ...
          if (currentSize >= memoryThreshold) {
            ...
            accountingLock.synchronized {
              if (!reserveUnrollMemoryForThisThread(amountToRequest)) {
                ...
                /**
                 * 反复循环判断,只要数据没有写入内存,并且可以尝试往内存中写,
                 * 如果内存不够,那么调用ensureFreeSpace方法,尝试清空一些内存空间
                 */
                if (spaceToEnsure > 0) {
   ...
   */
  /**
   * 有限放入内存,如果内存空间不足,尝试移除部分旧数据,再将block数据放入
   */
  private def tryToPut(
    ...
    accountingLock.synchronized {
      /**
       * 调用ensureFreeSpace方法,首先判断内存是否有足够的空间,如果内存
       * 空间不足,此时会将部分数据调用dropFromMemory方法尝试将其写入内存,
       * 但是如果持久化级别不支持写入磁盘,那么数据将会被丢弃
       */
      ...
      if (actualFreeMemory + selectedMemory >= space) {
        ...
        for (blockId <- selectedBlocks) {
          ...
          if (entry != null) {
            ...
            // 将数据移除内存,并且尝试将其写入到磁盘,但是如果block的持久化级别没有设置将其写入磁盘,那么数据将丢弃
            val droppedBlockStatus = blockManager.dropFromMemory(blockId, data)
            droppedBlockStatus.foreach { status => droppedBlocks += ((blockId, status)) }
          }
        }

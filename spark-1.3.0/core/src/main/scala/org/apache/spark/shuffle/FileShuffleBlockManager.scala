...
class FileShuffleBlockManager(conf: SparkConf)
   ...
   */
  /**
   * 为每个map task获取一个ShuffleWriterGroup
   */
  def forMapTask(shuffleId: Int, mapId: Int, numBuckets: Int, serializer: Serializer,
    ...
    new ShuffleWriterGroup {
      ...
      /**
       * 针对不同的shuffle模式,具有不同的方式
       * 1.普通Shuffle模式
       *   没有开启consolidate机制,会为每一个bucketId创建一个writer,也就是拿取一个新的文件句柄
       * 2.优化的Shuffle模式
       *   也就是开启了consolidate机制,即consolidateShuffleFiles=true,会为bucket获取一个ShuffleGroup的writer
       */
      val writers: Array[BlockObjectWriter] = if (consolidateShuffleFiles) {
        fileGroup = getUnusedFileGroup()
        Array.tabulate[BlockObjectWriter](numBuckets) { bucketId =>
          /**
           * 用shuffleId、mapId、bucketId(reduceId)生成一个唯一的ShuffleBlockId,
           * 然后,用bucketId调用ShuffleFileGroup的apply方法,为bucket获取一个ShuffleFileGroup
           */
          val blockId = ShuffleBlockId(shuffleId, mapId, bucketId)
          /**
           * 然后,用BlockManager的getDiskWriter方法,针对ShuffleFileGroup获取一个Writer。
           * 因此,如果开启了consolidate机制,针对于每个bucket会获取一个ShuffleFileGroup的writer,
           * 而不是一个独立的ShuffleBlockFile的Writer。也就是所说的多个ShuffleMapTask输出数据合并
           */
        ...
        /**
         * 普通Shuffle操作
         * 没有开启consolidate机制,对于每一个shuffleMapTask输出的bucket,那么都会获取一个单独的shuffkeBlockFile
         */
        Array.tabulate[BlockObjectWriter](numBuckets) { bucketId =>
          // 生成shuffleBlockId
          val blockId = ShuffleBlockId(shuffleId, mapId, bucketId)
          // 调用blockManager的diskBlockManager,获取一个写入本地磁盘文件的blockFile
          ...
          // 调用blockManager的getDiskWriter方法,针对blockFile生成writer

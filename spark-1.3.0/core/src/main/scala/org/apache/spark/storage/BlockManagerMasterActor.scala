 ...
 */
/**
 * BlockManagerMasterActor负责维护各个executor的BlockManager的元数据,
 * 如BlockManagerInfo、BlockStatus
 */
private[spark]
class BlockManagerMasterActor(val isLocal: Boolean, conf: SparkConf, listenerBus: LiveListenerBus)
  ...
  /**
   * 映射block manager id到block manager info,
   * 也就是说BlockManagerMaster要负责维护每个BlockManager的BlockManagerInfo
   */
  private val blockManagerInfo = new mutable.HashMap[BlockManagerId, BlockManagerInfo]

  // Mapping from executor ID to block manager ID.
  /**
   * 映射executor id到block manager id,
   * 也就是说, 每个executor与另一个BlockManager相关联
   */
  ...
  private def removeBlockManager(blockManagerId: BlockManagerId) {
    // 根据blockManagerId获取BlockManagerInfo
    val info = blockManagerInfo(blockManagerId)

    // Remove the block manager from blockManagerIdByExecutor.
    // 从blockManagerIdByExecutor移除blockManagerId对应的executorId
    ...
    // 遍历BlockManagerInfo所有的block块
    while (iterator.hasNext) {
      // 清空BlockManagerInfo内部block的BlockStatus信息
  ...
  private def removeExecutor(execId: String) {
    ...
    // 获取executorId对应的BlockManagerInfo, 然后调用removeBlockManager方法
  ...
  private def register(id: BlockManagerId, maxMemSize: Long, slaveActor: ActorRef) {
    ...
    // 没有缓存该BlockManager信息,即没有注册
    if (!blockManagerInfo.contains(id)) {
      /**
       * 根据block manager对应的executor id,找到对应的BlockManagerInfo。
       * 作用：安全判断
       * 分析：因为如果blockManageInfo map没有blockManagerId, 那么同步的
       *       blockManagerByExecutor也是没有blockManagerId,如果有则做一些清理工作
       */
      blockManagerIdByExecutor.get(id.executorId) match {
        case Some(oldId) =>
          ...
          // 移除缓存executorId对应的blockManagerInfo
      ...
      // 缓存executorId到BlockManagerId的映射到blockManagerIdByExecutor中
      blockManagerIdByExecutor(id.executorId) = id
      
      // 创建BlockManagerInfo,并且缓存到blockManagerId对应的blockManagerInfo map中
  ...
  /**
   * 更新BlockInfo
   * 如果BlockManager的block发生变化,将发生updateBlockInfo请求到BlockManagerMaster,然后更新BlockInfo信息
   */
  private def updateBlockInfo(
    ...
    /**
     * block可能会分布在多个BlockManager中。
     * 如果将StorageLevel设置成_n,那么需要将block复制(n-1)份,缓存到其他BlockManager上。
     * blockLocation保存了每个blockId对应的BlockManagerId的set集合。
     * locationss是用set存储blockManagerId信息的,因此BlockLocations中的信息将被更新。
     */
...
/**
 * BlockManager的BlockManagerInfo,
 * 相当于是,BlockManager的元数据
 */
private[spark] class BlockManagerInfo(
  ...
  // BlockManagerInfo管理每个BlockManager内部Block的block-blockStatus的映射
  ...
  def updateBlockInfo(
    ...
    /**
     * 给block创建一份BlockStatus, 然后根据其持久化级别, 对相应内存资源进行计算。
     */
    if (storageLevel.isValid) {
      ...
      // 如果StorageLevel是非法的, 并且已缓存, 那么将其移除
      val blockStatus: BlockStatus = _blocks.get(blockId)

...
/**
 * 1.实现高层次面向stage的调度
 * 2.为job计算一个stage的DAG(有向无环图),追踪RDD和stage的输出是否被物化(写入磁盘或内存),寻找最优的调度运行Job
 * 3.将提交stages作为tasksets提交到底层的taskscheduler上,在集群上运行
 * 
 * 4.此外,除了处理stage的DAG,DAGScheduler还负责决定根据当前缓存运行每个task的最佳位置,并将这些最佳位置提交给底层的TaskScheduler
 * 5.如果由于shuffle输出文件失败导致的丢失,那么先前运行的stage会被重新提交
 * 6.如果是由于stage内部失败,会被TaskScheduler重试多次处理,超过一定次数该stage会被取消
 */
/**
...
class DAGScheduler(
  ...
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    ...
    // We are manually maintaining a stack here to prevent StackOverflowError
    // caused by recursively visiting
    val waitingForVisit = new Stack[RDD[_]] // stack
      ...
      if (!visited(rdd)) {
        ...
        if (getCacheLocs(rdd).contains(Nil)) {
           /**
             * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
             *  遍历rdd的依赖
             *  其实,针对每一种shuffle操作,比如groupByKey、reduceByKey、countByKey
             *  底层对应了3个RDD：MapPartitionsRDD、ShuffleRDD、MapPartitionsRDD
             * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
             */
          for (dep <- rdd.dependencies) {
            dep match {
              ...
              case shufDep: ShuffleDependency[_, _, _] =>
                /**
                 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                 *  那么,将使用宽依赖的rdd,创建一个stage,并且会将isShuffleMap设置为true
                 *  默认,最后一个state不是shuffleMap Stage
                 *  但是,finalStage之前所有的stage都是shuffleMap stage
                 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                 */
      ...
      properties: Properties = null)
  {
    ...
    if (finalStage != null) {
      ...
      //   也就是说,这个job的最后一个stage就是finalStage
      ...
      if (shouldRunLocally) {
        ...
        //   导致,第一个stage被提交,其他所有stage都放入waitingStages队列中.
        submitStage(finalStage)
        
        /**
         * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         * 1.stege的重要性
         * 必须对stage划分算法有很清晰认识,明确spark application被划分为哪些job,每个job被划分为哪些stage.
         * 每个stage包含了哪些代码。只有直到每个stage包含了哪些代码之后,如果发现某个stage执行特别慢,或者一直
         * 报错,才能针对该stage对应的代码,去排查问题,或者性能调优.
         * 
         * 2.划分
         * 1)从finalStage倒推
         * 2)通过宽依赖划分stage
   ...
   * 提交Stage
   * stage划分算法的入口
   * 由submitStage和getMissingParentStages共同组成划分的
   */
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        // 如果当前stage没有父stage,则提交这个stage
        if (missing == Nil) { // 直到,没有父stage,此时首先提交第一个stage,其余的stage此时全部都在waitingStages队列中
          logInfo("Submitting " + stage + " (" + stage.rdd + "), which has no missing parents")
          submitMissingTasks(stage, jobId.get)
        } else {
          // 递归调用submitStage方法,提交父stage,stage划分算法的精髓
          for (parent <- missing) {
            submitStage(parent)
          }
          // 将当前stage放入等待执行的stage队列waitingStages中
          waitingStages += stage
        }
      }
    } else {
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    val tasks: Seq[Task[_]] = if (stage.isShuffleMap) {
      partitionsToCompute.map { id =>
        // 为每个partition创建一个task,为task计算最佳位置
        val locs = getPreferredLocs(stage.rdd, id)
        val part = stage.rdd.partitions(id)
        // 对于finalStage除外的stage, 它的isShuffleMap都是true, 都会创建一个ShuffleMapTask
        new ShuffleMapTask(stage.id, taskBinary, part, locs)
      }
    } else {
      // 对于finalStage. 创建ResultTask
      val job = stage.resultOfJob.get
      partitionsToCompute.map { id =>
    ...
    if (tasks.size > 0) {
      ...
      // 针对stage的task,创建TaskSet对象,调用submitTasks提交TaskSet
      // 默认情况下,standalone模式,使用的是TaskSchedulerImpl, 而TaskScheduler只是一个trait
   ...
   * 也即,从stage最后一个RDD开始,查找RDD partition被cache或者checkpoint的位置,即为最佳位置
   * 因为,task就在最佳位置上执行,不需要重复计算之前的RDD
   */
    ...
    : Seq[TaskLocation] =
  {
    ...
    // 递归调用自己,寻找RDD的父RDD,查看对于的partition是否缓存或者checkpoint
    ...
    // 如果stage,  从最后的RDD到开始的RDD,partition都没有被缓存或者checkpoint
    // 那么,task最佳位置就是Nil
    Nil
  }

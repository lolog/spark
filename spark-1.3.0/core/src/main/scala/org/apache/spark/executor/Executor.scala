  ...
  extends Logging
{
  ...
  def launchTask(
    ...
    // 对于每个task,都会创建一个TaskRunner, TaskRunner继承java的Runnable接口
  ...
  class TaskRunner(
    ...
    override def run() {
      ...
      try {
        ...
        // 然后,通过网络通信,将需要的文件、资源、jar文件进行拷贝
        updateDependencies(taskFiles, taskJars)
        /**
         * 反序列化task数据
         * 
         * 问题：为何需要用到java的ClassLoader呢？
         * 解答：需要加载JAR的class文件,然后创建类对象、加载指定上下文等等资源
         */
        ...
        /**
         * 执行task
         * 1.如果后面还是一个ShuffleMapTask,那么会联系MapOutputTracker,
         *   获取上一个ShuffleMapTask的输出位置,然后通过网络拉取数据
         *   ResultTask也是如此
         * 2.value的类型：对于ShuffleMapTask来说,是MapStatus类型,包含了ShuffleMapTask计算的数据和输出的位置。
         */
        ...
        /**
         * 计算task metrics的统计信息
         * 包括：运行了时间、反序列化时间、JVM gc时间、结果序列化时间
         */
        for (m <- task.metrics) {
        ...
        /**
         * 核心
         * 调用了Executor中CoarseGrainedExecutorBackend的statusUpdate方法
         */
  ...
  private def updateDependencies(newFiles: HashMap[String, Long], newJars: HashMap[String, Long]) {
    ...
    /**
     * task是在一个CoarseGrainedExecutorBackend进程中并发运行的,
     * 执行作业的时候,需要访问一些共享的资源,可能会导致线程安全问题
     * 
     * 问题：为何需要进行多线程并发访问的同步？
     * 解答：因为对currentFiles等共享变量进行写操作
     */
    synchronized {
      ...
      for ((name, timestamp) <- newFiles if currentFiles.getOrElse(name, -1L) < timestamp) {
        ...
        // 调用Utils.fetchFile方法,通过网络通信从远程拉取数据

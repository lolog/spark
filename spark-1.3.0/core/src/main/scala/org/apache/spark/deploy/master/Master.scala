...
private[spark] class Master(
    ...
    case ExecutorStateChanged(appId, execId, state, message, exitStatus) => {
      ...
      execOption match {
        case Some(exec) => {
          ...
          // 向该executor对应的driver发送ExecutorUpdated消息
          ...
          if (ExecutorState.isFinished(state)) {
            ...
            // 非正常退出,会继续重新调度,限制重试调度的次数
            if (!normalExit) {
              if (appInfo.incrementRetryCount() < ApplicationState.MAX_NUM_RETRY) {
                schedule()
              } else {
                // executor反复调度失败,那么application则启动失败
  ...
  def completeRecovery() {
    ...
    /** -----------------------------------------------------------------------------------------------------
     * 没有出现故障的Application和Worker会事先将状态修改为UNKNOWN,然后将其状态发送给master.
     * 遍历分别调用removeWorker和finishApplication方法,对可能出现故障,或者已经挂掉的Application和Worker进行清理.
     * 清理的机制：1.从内存缓存清理
     *            2.从相关组件内存缓存中移除
     *            3.从持久化存储中移除
     * ------------------------------------------------------------------------------------------------------ **/
  ...
  def canUse(app: ApplicationInfo, worker: WorkerInfo): Boolean = {
    /**
     * worker.memoryFree         worker剩余内存
     * app.desc.memoryPerSlave   Application需要的内存
     * worker没有启动过该app
     */
  ...
  private def schedule() {
    ...
    // Random.shuffle原理：对传入的元素进行随机的打乱,取出workers中所有之前注册上来的worker
     ...
     * -----------------------------------------------------------------------------------------------------
     *  首先调度driver
     *  问题：为什么需要调度driver, 什么情况下回注册driver,并且会导致driver被调度
     *  答案：只有用yarn-cluster模式提交的时候,才会注册driver。
     *        因为standalone和yarn-client模式,都会在本地直接启动driver,而不会注册driver,更不可能让master调度driver了
     *        
     *  driver的调度机制
     *  遍历waitingDrivers
     * -----------------------------------------------------------------------------------------------------
    ...
    for (driver <- waitingDrivers.toList) { // iterate over a copy of waitingDrivers
      ...
      var numWorkersVisited = 0
       // while条件,numWorkersVisited < numWorkersAlive是什么意思呢
      // 即,只要还有活着的worker没有遍历到,那么就会继续继续遍历吗,对不对?
      // 而且,意思是当前这个driver还没有被启动,也就是launched=false
      while (numWorkersVisited < numWorkersAlive && !launched) {
        val worker = shuffledAliveWorkers(curPos)
        numWorkersVisited += 1
        /**
         * worker.memoryFree   worker的空闲内存
         * driver.desc.mem     driver需要的内存
         * worker.coresFree    worker空闲的cou数量
         * driver.desc.cores   driver需要的cpu数量
         */
    ...
    /**
     * Application的调度机制[核心,重点]
     * 首先,Application的调度算法有：spreadOutApps和非spreadOutApps
     */
    if (spreadOutApps) {
      // Try to spread out each app among all the nodes, until it has all its cores
      // 遍历waitingApps中的ApplicationInfo, 并且Application还有需要待调度的core
      for (app <- waitingApps if app.coresLeft > 0) {
        // 从workers中,过滤：状态为alive、可以被Application使用的worker, 排序：剩余cpu数量倒序
        val usableWorkers = workers.toArray.filter(_.state == WorkerState.ALIVE)
          .filter(canUse(app, _)).sortBy(_.coresFree).reverse
        val numUsable = usableWorkers.length
        // 创建一个空数组,存储需要分配给每个worker的cpu数量
        val assigned = new Array[Int](numUsable) // Number of cores to give on each node
        // 获得到底需要分配多少cpu, 取app剩余需要分配的cpu数量,和worker总共可用cpu数量的最小值
        var toAssign = math.min(app.coresLeft, usableWorkers.map(_.coresFree).sum)
        
        // ++++++++++++ 算法：将每个app需要启动的executor平均分配到各个worker上去  +++++++++++++//
        var pos = 0
        // 只要待分配的cpu没有分配完,就继续进行分配
        while (toAssign > 0) {
          // usableWorkers(pos).coresFree      pos位置的worker的cpu数量
          // assigned(pos)                     已分配的cpu数量
          ...
          // 算法：如果该worker的cpu core不够,那么待分配的进行下一个worker分配
          pos = (pos + 1) % numUsable
        }
        
        // Now that we've decided how many cores to give on each node, let's actually give them
        // 给每个worker分配完application需要的cpu core之后,遍历worker
        for (pos <- 0 until numUsable) {
          // 只要判断之前给这个worker分配到了core
          if (assigned(pos) > 0) {
            /**
             * 首先,在application内存结构中,创建ExecutorDesc对象,封装了内存和CPU核数.
             * 
             * spark-submit提交的参数
             * --executor-core         每个executor的核数。在yarn或者standalone下使用
             * --driver-cores          Driver的核数,默认是1。在yarn集群模式下使用
             * --num-executors         启动的executor数量。默认为2。在yarn下使用
             * 那么,启动的数量可能和spark-submit指定的数量有差异,由worker的CPU核数和提交参数共同决定
             */
      ...
      /**
       * 非spreadOutApps调度算法
       * 目的：尽可能地将executor集中分配到一个worker上运行,调度算法和spreadOutApps正好相反
       */
      // Pack each app into as few nodes as possible until we've assigned all its cores
      for (worker <- workers if worker.coresFree > 0 && worker.state == WorkerState.ALIVE) {
        // 首先将application executor尽可能地分配到一个worker上运行
        for (app <- waitingApps if app.coresLeft > 0) {
  ...
  def launchExecutor(worker: WorkerInfo, exec: ExecutorDesc) {
    ...
    // 将executor加入到worker缓存
    worker.addExecutor(exec)
    // 向worker发送LaunchExecutor消息启动executor
    worker.actor ! LaunchExecutor(masterUrl,
      exec.application.id, exec.id, exec.application.desc, exec.cores, exec.memory)
    // 向该executor application的driver发送ExecutorAdded消息,将该executor添加到driver中
  ...
  def launchDriver(worker: WorkerInfo, driver: DriverInfo) {
     ...
     * --------------------------------------------------------------
     * 将driver加入worker的缓存
     * worker使用的内存和cpu数量 + driver需要的内存和cpu数量
     * --------------------------------------------------------------
     */
    worker.addDriver(driver)
    // 同时, 将worker加入driver的缓存
    driver.worker = Some(worker)
    // 然后,调用worker的actor给worker发送LaunchDriver消息,让worker启动driver

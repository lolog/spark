 ...
 */
/**
 * 1.在TaskSchedulerImpl中,TaskSetManager对TaskSet的任务进行调度。
 * 2.TaskSetManager负责对task进行跟踪和监视,如：当task失败的话,会对task进行重试,直到超过一定的次数限制。并且通过延迟调度,为该TaskSet处理本地化调度机制。
 * 3.主要接口是ResourceOffer,接口中TaskSet期望在一节点上运行任务,并且接收任务状态来监视task状态改变
 */

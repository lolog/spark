 ...
 * 主要提供了在本地或者远程存取数据的功能,支持内存、磁盘、堆外存储(tychyon)
 */
private[spark] class BlockManager(
  ...
  /**
   * BlockManager维护在内存中存储数据的一个个Block块,即blockId对应的Block块数据。
   * BlockInfo最大的作用：用于多线程并发访问同一个Block的同步监视器。
   */
  ...
  def initialize(appId: String): Unit = {
    /**
     * 1.3.0以前版本, BlockManager使用的时BlockManagerWorker来针对数据的传输。 1.3.0以后,更换为BlockTransferService。
     * 
     * 初始化, 用于远程block数据传输
     */
    ...
    /**
     * 为当前BlockManager创建一个BlockManagerId
     * 
     * executorId BlockManager关联一个Executor、BlockTransferService的hostname、以及BlockTransferService的port。
     * 因此,从BlockManagerId的初始化可以看出, 一个BlockManager是通过一个节点上的Executor来唯一标志的
     */
    ...
    // 使用BlockManagerMasterActor的引用,发送消息给BlockManagerMaster进行BlockManager的注册
  ...
  private def doGetLocal(blockId: BlockId, asBlockResult: Boolean): Option[Any] = {
    ...
    if (info != null) {
      /**
       * 对于所有的BlockInfo, 都会进行多线程并发访问的同步操作。
       * 
       * 所以,BlockInfo相当于是对一个Block, 用于作为多线程并发访问的同步监视器。
       */
      info.synchronized {
        ...
        // 内存级别, 比如MEMORY_ONLY、MEMORY_AND_DISK_SER, 尝试从MemoryStore获取数据
        if (level.useMemory) {
        ...
        if (level.useDisk) {
          ...
          if (!level.useMemory) {
            ...
            if (!level.deserialized || !asBlockResult) {
              ...
              /**
               * 同时使用了disk和memory级别,
               * 那么,从disk读取出来之后,将尝试将其存放到memory中
               */
  ...
  private def doGetRemote(blockId: BlockId, asBlockResult: Boolean): Option[Any] = {
    ...
    // 首先从BlockManagerMaster获取blockId对应的BlockManager的信息,随机打乱
    val locations = Random.shuffle(master.getLocations(blockId))
    // 遍历
    for (loc <- locations) {
      logDebug(s"Getting remote block $blockId from $loc")
      // 使用BlockTransferService进行异步网络获取,从远程将block数据download回来
   ...
   * 
   * 通过BlockManager获取数据的入口,首先从本地获取,如果本地没有需要的数据,再从远程获取。
   */
  ...
  private def doPut(
    ...
    // 为需要写入的block, 创建一个BlockInfo, 并将其放入到blockinfo map中
    val putBlockInfo = {
    ...
    putBlockInfo.synchronized {
      ...
      try {
        ...
        // blockStore - The type of storage to put these values intoen
        // 根据持久化级别,选择一种BlockStore, 如MemoryStore、DiskStore等
        ...
        if (putBlockStatus.storageLevel != StorageLevel.NONE) {
          ...
          if (tellMaster) {
            // 调用reportBlockStatus方法,将新写入的block数据发送到BlockManagerMasterActor,以便于进行block元数据的同步和维护
    ...
    // 持久化级别定义为_2这种后缀,意味着需要对block进行replica,传输到其他节点上       

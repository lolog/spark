  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverActor(sparkProperties: Seq[(String, String)]) extends Actor with ActorLogReceive {
    ...
    def makeOffers() {
      /**
       * 第一步.调用TaskSchedulerImpl的resourceOffers,执行分配算法,将各个task分配到executors上去
       * 第二步.分配好task到executor之后,执行自己的lanuchTasks,将分配的task发送到LanuchTask消息到对应的
       *        executor上去,由executor启动并执行task
       * 
       * 分析
       * 1.给resourceOffers传入的是什么?
       *   传入的是该Application所有可用的executor,并且将其封装成WorkerOffer,WorkerOffer代表了executor可用的CPU资源
       */
    ...
    /**
     * 根据算法的分配,在executor上启动相应的task
     */
    def launchTasks(tasks: Seq[Seq[TaskDescription]]) {
      for (task <- tasks.flatten) {
        // 首先,将每个executor需要执行的task信息,统一执行序列化操作

...
abstract class RDD[T: ClassTag](
   ...
   */
  /**
   * 1.首先persist, 再进行checkpoint
   * 2.当storageLevel不为StorageLevel.NONE时, 从cacheManager获取数据。 
   *   如果从BlockManager没有获取到数据(第一次执行),RDD数据将被计算,再调用
   *   CacheManager的putInBlockManager方法将数据进行持久化
   * 2.RDD的job结束之后,单独启动job进行checkpoint操作,然后由回到iterator方法,
   *   那么,此时持久化级别不为空,正常情况下,默认从BlockManager直接读取持久化数据。
   * 3.非正常情况下,持久化数据丢失,那么将调用computeOrReadCheckpoint方法,
   *   如果RDD的isCheckpointed为true,将使用它的父RDD iterator方法(即,从checkpoint外部文件系统中读取数据)
   */
  final def iterator(split: Partition, context: TaskContext): Iterator[T] = {
    /**
     * storageLevel不为None,也就是RDD已被持久化。
     * 那么,不直接从父RDD执行算子计算新的RDD的partition, 优先尝试使用CacheManager拿取持久化数据。
     */

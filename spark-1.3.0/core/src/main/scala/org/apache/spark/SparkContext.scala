...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
   ...
   */
  /**
   * 1.hadoopFile会创建一个HadoopRDD,(key, value)为(为本行的offset,文本行)
   * 2.对HadoopRDD调用map剔除key,返回MapPartitionsRDD
   * 3.MapPartitionsRDD内部元素就是一行一行的文本行
   */
  ...
  def runJob[T, U: ClassTag](
    ...
    // 调用SparkContext初始化时创建的DAGScheduler的runJob方法
